    ...
    $.ajax({
        ...
        success: function (msg) {
            ...
            $.each(msg.result.priceList, function (index, currentPriceItem) {
                ...
                html += '<td class="td-b td_pos"><input class="input_1 monopolize" id="input_price_' + index + '" onfocus="showSpan(this,' + provinceId + ',' + index + ')" value="' + price + '" ' + (currentPriceItem.throwable == 'N' ? 'disabled' : '') + ' /></td>';
                //TODO 设置是否可投方，需要该功能的时候再打开  BY liujie136
                // var text = currentPriceItem.throwable == 'N' ? '不可投' : '可投';
                // var checked = currentPriceItem.throwable == 'N' ? "" : "checked";
                // var htmlThrowable = '<label id="lab_throwable_' + index + '" for="input_throwable_' + index + '">' + text + '</label><input id="input_throwable_' + index + '" type="checkbox" style="display: none" ' + checked + ' />';
            ...
            //     var text = $(this).prop("checked") ? '可投' : '不可投';
...
function showSpan(curInput, provinceId, index) {
    if (index != lastShowSpanIndex) {
        var spanStr = '<a><span id="span_' + index + '" class="main_span_3" onclick="setToWholeProvince(this,' + provinceId + ',' + index + ')">同步修改该省数据</span></a>';
...
function setToWholeProvince(curInput, provinceId, index) {
    ...
    if (!validatePrice(newPrice)) {
        ...
        for (var tmpIndex = startIndex; tmpIndex < endIndex; tmpIndex++) {
            ...
            var inputElement = document.getElementById("input_price_" + tmpIndex);
...
function submitPrice() {
    if (confirm('您确定修改这个资源位独占价格吗?')) {
        ...
        for (var tmpIndex = 0; tmpIndex < originalPriceItems.length; tmpIndex++) {
            var newPrice = document.getElementById("input_price_" + tmpIndex).value;
            if (!validatePrice(newPrice)) {
                alert("请输入数字[0~9999999999999.99]，最多保留小数点后两位");
