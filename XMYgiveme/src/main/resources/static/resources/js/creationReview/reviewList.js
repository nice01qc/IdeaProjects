...
;(function(window, document, $) {
    ...
    var reviewListUrl = '/adxReviewManager/getWaitReviewList',
        getAllDspUrl = '/adxReviewManager/getAllDsp',
        reviewUrl = '/adxReviewManager/operateReview';

    var passedText = '<span class="glyphicon glyphicon-ok"></span>审核通过',
        retreatedText = '<span class="glyphicon glyphicon-remove"></span>审核退回';
    ...
    var topBanner = '<div class="table-top-banner">' +
        ...
        '<div class="pull-right">查询到<span class="total-count-no">--</span>条未审核</div></div>';
    ...
    var reviewListCols = [{
        ...
        render: function(v, item, i) {
            return '<div style="height: 100%;">' +
                ...
                '<img width="350px" style="margin-bottom: 15px;" src="' + item.url + '"></div>';
        }
    }, {
        title: '链接展示',
        name: 'links',
    ...
    function girdLoadSuccessHandler() {
        ...
        $('#batch-pass-btn').on('click', function() {
            var _this = $(this);
        ...
        $('#batch-retreat-btn').on('click', function() {
            var _this = $(this);
            reviewListComponent.getSelectedData(function(selectedData) {
                if (selectedData.data.length > 0) {
