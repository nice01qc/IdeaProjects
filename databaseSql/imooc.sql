...
INSERT INTO `customers` VALUES ('10005', 'E Fudd', '4545 53rd Street', 'Chicago', 'IL', '44444', 'USA', 'E Fudd', null);
...
-- Records of student
-- ----------------------------
INSERT INTO `student` VALUES ('1', '2017', 'Make', '239dk', 'man', '33', '2-5class', 'Jake', 'math', 'three');
...
-- ----------------------------
-- View structure for product1
-- ----------------------------
DROP VIEW IF EXISTS `product1`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `product1` AS select `customers`.`cust_name` AS `cust_name`,`customers`.`cust_address` AS `cust_address` from `customers` ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro1`(IN p_in int)
BEGIN
...
-- ----------------------------
-- Procedure structure for pro2
-- ----------------------------
DROP PROCEDURE IF EXISTS `pro2`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro2`(IN p_in int)
BEGIN
	SELECT p_in;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro3`(IN p_in int)
BEGIN
	SELECT p_in;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro4`(OUT x INT)
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro5`()
BEGIN
	SELECT cust_id AS id ,cust_name AS name ,cust_city AS city FROM customers;
...
-- ----------------------------
-- Procedure structure for pro6
-- ----------------------------
DROP PROCEDURE IF EXISTS `pro6`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `pro6`(IN x INT , OUT y INT)
BEGIN
