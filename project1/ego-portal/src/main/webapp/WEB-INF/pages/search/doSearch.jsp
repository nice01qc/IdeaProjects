...
<div id="doc">
	<div id="s_hdw">
		...
		<div class="mmenu">
			<div class="s_hd">
				<div id="s_search">
					<form action="" method="post" id="s_search_form" onsubmit="return false;">
					<input name="searchStr" id="searchStr" value="${searchStr }" type="text" class="search-input" />
					<input type="hidden" name="currentPage" id="currentPage" value="1"/>
					<input type="hidden" name="rows" id="rows" value="10"/>
					<input onclick="doSearch()" name="" type="image" src="${ctx}/static/images/btn_search.jpg" />
					</form>
				</div>
...
<script type="template" id="s_search_content_templ">
{{ for(i=0;i<it.length;i++){ }}
<li>
	<a href="#"><img src="http://${ftpHost}/{{=it[i].originalImg}}" width="170" height="160" alt="" /></a>
	<dl>
		<dt><a href="#">{{=it[i].goodsNameHl}}</a></dt>
		<dd>特价：<strong class="red">￥{{=it[i].marketPrice}}</strong></dd>
		<dd><span class="startotal"></span></dd>
		<dd><a class="addcat" onclick="addToCart('{{=it[i].goodsId}}','{{=it[i].goodsName}}','{{=it[i].marketPrice}}','{{=it[i].originalImg}}')" href="javascript:void(0);">加入购物车</a></dd>
	</dl>
</li>
{{ } }}
</script>
<script type="template" id="s_search_page_templ">
<div class="f-r pagination">
	{{ if(it.hasPre){ }}
	<a href="javascript:void(0);" onclick="doSearch('{{=it.currentPage-1}}')">&lt;上一页  </a>
	{{ } }}
	
	{{ for(i=1;i<=it.total;i++){ }}
		{{ if(i==it.currentPage){ }}
	...
	{{ } }}
	<div class="yepage">到第<input class="stext" type="text" name="" id="jumpPage" value="" />页<input onclick="jumpPage()" class="btnimg" type="submit" name="" id="" value="" /></div>
</div>
</script>
<script type="text/javascript">
	$(function(){
	...
	function doSearch(currentPage){
		...
		$.ajax({
			url:"${ctx}/search/searchGoodsInfo",
			type:"POST",
			...
			success:function(res){
				...
				$("#rows").val(res.pageSize);
				//调用模板
				var content_templ = doT.template($("#s_search_content_templ").text());
				$("#s_search_content").html(content_templ(res.result));
				
				var page_temp = doT.template($("#s_search_page_templ").text());
				$("#s_search_page").html(page_temp(res));
