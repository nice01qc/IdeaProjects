...
<div id="doc">
	...
	<div id="s_bdw">
		<div id="s_bd">
			...
			<div class="cartlist">
				<form method="POST" action="${ctx }/order/savePreOrder" >
				<table width="100%">
					<tr>
						<th>购物车中的商品</th><th>EGO价</th><th>购买数量</th><th>订单时间</th><th>操作</th>
					</tr>
					<!-- 总金额 -->
					<input type="hidden" value="${allAmount }" id="goodsAllamount" name="goodsAllamount"/>
					<c:forEach var="cart" items="${carList }" varStatus="status">
					<input type="hidden" value="${cart.goodsId }" name="preOrderList[${status.index }].goodsId"/>
					<input type="hidden" value="${cart.goodsName }" name="preOrderList[${status.index }].goodsName"/>
					<input type="hidden" value="${cart.goodsImage }" name="preOrderList[${status.index }].goodsImage"/>
					<input type="hidden" value="${cart.goodsPrice }" name="preOrderList[${status.index }].goodsPrice"/>
					<tr bgcolor="#fffaf1">
						<td>
							<a href="#"><img class="smallpic" src="http://${ftpHost }/${cart.goodsImage }" width="80" height="80" /></a>
							<a href="#">${cart.goodsName }</a>
						</td>
						<td>￥<strong class="red" id="goods_price_${cart.goodsId }">${cart.goodsPrice }</strong></td>
						
						<td>
							<div class="addinput">
								<input type="text" name="preOrderList[${status.index }].goodsNum" value="${cart.goodsNum }" id="qty_item_${cart.goodsId }" onKeyUp="setAmount.modify('#qty_item_${cart.goodsId }')" class="stext"/>
                        		<a class="add" onClick="setAmount.add('#qty_item_${cart.goodsId }')" href="javascript:void(0)"></a>
								<a class="reduce" onClick="setAmount.reduce('#qty_item_${cart.goodsId }')" href="javascript:void(0)"></a>
							</div>
						</td>
						<td>
							<fmt:formatDate value="${cart.addTime }" pattern="yyyy-MM-dd hh:mm:ss" />
						</td>
					...
					<tr>
						...
						<td align="right" colspan="5">
							<p>共<font id="cart_num" color="red">3</font>件商品</p>
							<p style="margin-top:10px;font-size:14px;">总计金额(不含运费)：￥<strong style="font-size:18px;color:#d80000;" id="goods_allamount">${allAmount }</strong> </p>
						</td>
					</tr>
					<tr>
...
<script type="text/javascript">
	...
	var setAmount = {
		...
		amount:function(obj, mode) {
			//商品价格编号
			var goodsPriceId = obj.replace("qty_item_","goods_price_");
			//商品单价
			var goodsPrice = $(goodsPriceId).text();
			//总金额
			var allamount = $("#goods_allamount").text();
			var x = $(obj).val();
			if (this.reg(x)) {
				if (mode) {
					allamount = (parseFloat(allamount)+parseFloat(goodsPrice)).toFixed(2);
					x++;
				} else {
					if(x!=1){
						allamount = (parseFloat(allamount)-parseFloat(goodsPrice)).toFixed(2);
			...
			$("#goods_allamount").text(allamount);
			$("#goodsAllamount").val(allamount);
			return x;
		},
		reduce:function(obj) {
