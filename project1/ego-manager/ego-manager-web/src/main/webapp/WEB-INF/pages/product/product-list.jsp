...
<div class="wrapper">
  <div class="breadcrumbs" id="breadcrumbs">
    <ol class="breadcrumb">
      ...
      <li>
        <a href="javascript:void();">商品列表</a></li>
    </ol>
  </div>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title">
          <i class="fa fa-list"></i>商品列表</h3>
      </div>
      <div class="panel-body">
        <div class="navbar navbar-default">
          <form action="" id="search-form2" class="navbar-form form-inline" method="post" onsubmit="return false">
            <div class="form-group">
              <select name="catId" id="cat_id" class="form-control">
                ...
                <option value="455">Z 钻石</option></select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <select name="pageSize" class="form-control" onchange="resetCurrent()">
            ...
            <div class="form-group">
              ...
              <div class="input-group">
                <input name="goodsName" value="" placeholder="搜索词" id="input-order-id" class="form-control" type="text"></div>
            </div>
            <!--排序规则-->
            ...
            <button type="button" onclick="location.href='${ctx}/product/add'" class="btn btn-primary pull-right">
              <i class="fa fa-plus"></i>添加新商品</button>
            </form>
        </div>
        <div id="ajax_return">
              ...
              <table class="table table-bordered table-hover">
                <thead>
                  <tr>
                    ...
                    <td class="text-right">
                      <a href="javascript:sort('goods_id');">ID</a></td>
                    <td class="text-left">
                      <a href="javascript:sort('goods_name');">商品名称</a></td>
                    <td class="text-left">
                      <a href="javascript:sort('goods_sn');">货号</a></td>
                    <td class="text-left">
                      <a href="javascript:sort('cat_id');">分类</a></td>
                    <td class="text-left">
                      <a href="javascript:sort('shop_price');">价格</a></td>
                    <td class="text-left">
                      <a href="javascript:void(0);">库存</a></td>
                    <td class="text-center">
                      <a href="javascript:sort('sort');">排序</a></td>
                    <td class="text-center" style="width:135px;">操作</td></tr>
                </thead>
                <tbody id="goodsContext">
                </tbody>
              </table>
            </div>
...
<!-- /.content-wrapper -->
<!-- 商品列表模板 -->
<script type="template" id="goodsTempl">
{{ for(i=0;i<it.length;i++){ }}
<tr>
	<td class="text-center">
	 <input name="" value="" type="hidden"></td>
	...
	<td class="text-left">
	  <input onkeyup="this.value=this.value.replace(/[^\d.]/g,'')" onpaste="this.value=this.value.replace(/[^\d.]/g,'')" 
		onchange="ajaxUpdateField(this);" name="store_count" size="4" data-table="goods" data-id="143" value="984" type="text">
	</td>
	<td class="text-center">
	  <input onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onpaste="this.value=this.value.replace(/[^\d]/g,'')" 
		onchange="updateSort('goods','goods_id','143','sort',this)" size="4" value="{{=it[i].sort}}" type="text">
	</td>
	<td class="text-center">
	  <a href="" class="btn btn-primary" title="编辑">
		<i class="fa fa-pencil"></i>
	  </a>
	  <a href="" onclick="del('143')" class="btn btn-danger" title="删除">
...
<!-- 分页模板 -->
<script type="template" id="pageTempl">
{{ if(it.hasPreviousPage == true){ }}
	<li id="example1_next" class="paginate_button previous">
		<a class="previous" data-p="2" href="javascript:goPage({{=it.prePage}})">上一页</a>
	</li>
{{ } }}


{{ for(i=1;i<=it.pages;i++){ }}
	<li class="paginate_button 
		{{ if(i == it.pageNum){ }}
		active
		{{ } }}
	">
		<a tabindex="0" data-dt-idx="1" aria-controls="example1" data-p="1" href="javascript:goPage({{=i}})">{{=i}}</a>
	</li>
{{ } }}

{{ if(it.hasNextPage == true){ }}
	<li id="example1_next" class="paginate_button next">
		<a class="next" data-p="2" href="javascript:goPage({{=it.nextPage}})">下一页</a>
	</li>
{{ } }}
</script>
<script>
...
function ajax_get_table(form, page) {
  $.ajax({
    type: "POST",
    url: "${ctx}/product/getData",
    data: $('#search-form2').serialize(),
    dataType:"JSON",
    success: function(data) {
    	//调用模板
    	var goodsTemp = doT.template($("#goodsTempl").text());
    	$("#goodsContext").html(goodsTemp(data.pageInfo.list));
    	//分页模板调用
    	var pageTempl = doT.template($("#pageTempl").text());
    	$("#pageContent").html(pageTempl(data.pageInfo));
...
function del(id) {
  ...
  $.ajax({
    ...
    success: function(v) {
      var v = eval('(' + v + ')');
      if (v.hasOwnProperty('status') && (v.status == 1)) ajax_get_table('search-form2', cur_page);
      else layer.msg(v.msg, {
        icon: 2,
        time: 1000
      }); //alert(v.msg);
    }
