		...
		<div id="s_bd">
			...
			<div class="cartlist">
				...
				<table width="100%">
					...
					<input type="text" value="${cart.goodsId }" name="preOrderList[${status.index }].goodsId"/>
					<input type="text" value="${cart.goodsName }" name="preOrderList[${status.index }].goodsName"/>
					<input type="text" value="${cart.goodsImage }" name="preOrderList[${status.index }].goodsImage"/>
					<input type="text" value="${cart.goodsPrice }" name="preOrderList[${status.index }].goodsPrice"/>
					<tr bgcolor="#fffaf1">
						<td>
							<a href="#"><img class="smallpic" src="http://192.168.88.10/${cart.goodsImage }" width="80" height="80" /></a>
...
<script type="text/javascript">
	...
	var setAmount = {
		...
		amount:function(obj, mode) {
			...
			if (this.reg(x)) {
				if (mode) {
					$("#goods_allamount").text((parseFloat(allamount)+parseFloat(goodsPrice)).toFixed(2));
					x++;
				} else {
					if(x!=1){
						$("#goods_allamount").text((parseFloat(allamount)-parseFloat(goodsPrice)).toFixed(2));
